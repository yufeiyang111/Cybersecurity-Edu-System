...
CREATE TABLE IF NOT EXISTS agent_audit_hypotheses (
    ...
    status ENUM('queued', 'active', 'needs_evidence', 'confirmed', 'rejected', 'stopped_for_budget') NOT NULL DEFAULT 'queued',
    planner_source VARCHAR(64) NOT NULL,
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_agent_audit_hypotheses_run FOREIGN KEY (run_id) REFERENCES agent_runs(id) ON DELETE CASCADE,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Harness V3 受控漏洞假设';

CREATE TABLE IF NOT EXISTS agent_audit_hypothesis_verdicts (
    id INT AUTO_INCREMENT PRIMARY KEY,
    hypothesis_id INT NOT NULL,
    verdict_version INT NOT NULL,
    verdict ENUM('confirm_candidate', 'request_evidence', 'reject_hypothesis', 'needs_more_evidence', 'stop_for_budget') NOT NULL,
    reason_summary VARCHAR(2000) NOT NULL,
    evidence_gaps_json JSON NULL,
    next_action_json JSON NULL,
    critic_version VARCHAR(64) NOT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_agent_audit_hypothesis_verdicts_hypothesis FOREIGN KEY (hypothesis_id) REFERENCES agent_audit_hypotheses(id) ON DELETE CASCADE,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Harness V3 Evidence Critic 结构化判定';
