<template>
  <section class="chart-panel"><header class="chart-head"><div class="chart-title"><BaseIcon name="chart" :size="17" /><strong>消耗分布</strong><span>总计：¥{{ Number(analytics?.summary?.total_cost || 0).toFixed(2) }}</span></div><div class="chart-toggle"><button :class="{ active: chartType === 'bar' }" @click="chartType = 'bar'"><BaseIcon name="chart" :size="14" />柱状图</button><button :class="{ active: chartType === 'area' }" @click="chartType = 'area'"><BaseIcon name="area" :size="14" />面积图</button></div></header><div v-if="loading" class="chart-loading">正在加载图表...</div><div v-else-if="!analytics?.trend?.length" class="chart-empty">当前时间范围暂无调用数据</div><div v-else ref="chartRef" class="chart" /></section>
</template>
...
const render = async () => { await nextTick(); if (!chartRef.value) return; if (!chart) chart = echarts.init(chartRef.value); const trend = props.analytics?.trend || []; const buckets = [...new Set(trend.map((item) => item.bucket))]; const models = [...new Set(trend.map((item) => item.model || '未知模型'))]; chart.setOption({ animationDuration: 650, animationEasing: 'cubicOut', color: colors, tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } }, legend: { bottom: 4, data: models, textStyle: { color: '#475569', fontSize: 12 } }, grid: { top: 18, left: 48, right: 18, bottom: 52 }, xAxis: { type: 'category', data: buckets, axisLabel: { color: '#94a3b8', fontSize: 11 }, axisLine: { lineStyle: { color: '#e2e8f0' } }, axisTick: { show: false } }, yAxis: { type: 'value', axisLabel: { color: '#94a3b8', fontSize: 11 }, splitLine: { lineStyle: { color: '#eef2f7' } } }, series: models.map((model, index) => ({ name: model, type: 'bar', stack: 'cost', barMaxWidth: 28, smooth: chartType.value === 'area', areaStyle: chartType.value === 'area' ? { opacity: .22 } : undefined, data: buckets.map((bucket) => trend.find((item) => item.bucket === bucket && (item.model || '未知模型') === model)?.cost || 0) })) }, true) }
