  ...
  <section class="chart-panel">
    <header class="chart-head">
      <div class="chart-title">
        ...
        <span>总 Token：{{ formatInteger(analytics?.summary?.total_tokens || 0) }}</span>
      </div>
      <div class="chart-toggle">
        ...
        <button :class="{ active: chartType === 'area' }" @click="chartType = 'area'">
          <BaseIcon name="area" :size="14" />
          面积图
        </button>
      </div>
    </header>
    <div v-if="loading" class="chart-loading">正在加载图表...</div>
    <div v-else-if="!analytics?.trend?.length" class="chart-empty">当前时间范围暂无调用数据</div>
  ...
  chart.setOption(
    {
      ...
      series: models.map((model, index) => ({
        ...
        symbolSize: 6,
        lineStyle: isArea ? { width: 2 } : undefined,
        areaStyle: isArea ? { opacity: 0.22 } : undefined,
        data: buckets.map((bucket) => trend.find((item) => item.bucket === bucket && (item.model || '未知模型') === model)?.tokens || 0)
      }))
    },
    true
  )
...
watch(
  () => [props.analytics, props.loading, chartType.value],
  render,
  { deep: true }
)
