import { ref } from 'vue'
import { qaAPI, knowledgeAPI, userAPI } from '@/api'
...
export function useProfileStats() {
  ...
  async function load() {
    loading.value = true
    try {
      const [qaCount, qaFav, knowledgeFav, recent, activity] = await Promise.all([
        ...
        userAPI.getActivity().catch(() => null)
      ])
      ...
      qaEvents.value = activity?.qa || []
      taskEvents.value = activity?.tasks || []
    } catch {
      questions.value = 0
      favorites.value = 0
      answers.value = 0
  ...
  return { loading, questions, favorites, answers, qaEvents, taskEvents, load }
}
