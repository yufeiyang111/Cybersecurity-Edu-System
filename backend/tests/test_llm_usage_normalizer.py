...
def test_normalize_reports_cache_field_even_when_value_is_zero():
    """LabexAgent semantics: a present cache field with value 0 is reported (MISS)."""
    normalized = normalize_usage(
        {
            "prompt_tokens": 100,
            "completion_tokens": 10,
    ...
    assert cache_status(
        normalized["cache_usage_reported"],
        cached_tokens=normalized["cached_tokens"],
        cache_write_tokens=normalized["cache_write_tokens"],
    ) == CACHE_STATUS_MISS


def test_normalize_flat_cache_field_zero_is_reported():
    normalized = normalize_usage(
        {
            "prompt_tokens": 100,
            "completion_tokens": 10,
